    ...
    getMobileBanking: async (req, res) => {
        ...
        try {
            ...
            if (status && !email) {
                ...
                const mobileBanking_request = await mobileBanking.find({ status: status }).sort({ "updatedAt": -1 }).populate("user", "name email -_id shope_name number ").limit(limit * 1).skip((page - 1) * limit);
            ...
            if (email && status) {
                ...
                const mobileBanking_request = await mobileBanking.find({ status: status, "user": id }).sort({ "updatedAt": -1 }).populate("user", "name email -_id shope_name number ").limit(limit * 1).skip((page - 1) * limit);
                res.status(200).json({ total: total.length, data: mobileBanking_request })
            };

            if (!status && !email) {
                const total = await mobileBanking.find()
                const mobileBanking_request = await mobileBanking.find().sort({ "updatedAt": -1 }).populate("user", "name email -_id shope_name number ").limit(limit * 1).skip((page - 1) * limit) //items.mobileBankingId
                res.status(200).json({ total: total.length, data: mobileBanking_request })
            };
            if (!status && email) {
                const total = await mobileBanking.find({ "user": id })
                const mobileBanking_request = await mobileBanking.find({ "user": id }).sort({ "updatedAt": -1 }).populate("user", "name email -_id shope_name number ").limit(limit * 1).skip((page - 1) * limit)
                res.status(200).json({ total: total.length, data: mobileBanking_request })
            };
        } catch (error) {
            res.status(400).json(error)
